...
/*
//320 设置环境（context）（不是很清楚jim_tcl解释器嘅工作原理）并注册 openocd(noinit init add_script_search_dir) server(略) gdb(略) log(略) transport(略) interface(略) target(略) flash(略) nand(略) pld(略) mflash(略) 模块的相关命令（在前面模块后面的圆括号()中表示的是命令行，<略>表示无最深入去看该模块有边啲命令行）

//322 注册 util 相关命令选项 ms；处理器名称：util_command_handlers

//325 注册 ioutil 相关命令选项 cat , trunc , cp , append_file , meminfo , rm , peek , poke , ls , mac , ip ；处理器名称：ioutil_command_handlers

//332 设置 cmd_ctx 数据结构中嘅 mode 字段为 COMMAND_CONFIG

//333 设置命令集输出处理器（亦即系话上面所有命令嘅输出内容都会通过该输出该输出处理器输出？），最后通过log.c中嘅log_printf()函数输出

//336 开始执行命令，大概执行过程如下：parse_cmdline_args();server_preinit();parse_config_file();server_init();command_run_line();server_loop();
